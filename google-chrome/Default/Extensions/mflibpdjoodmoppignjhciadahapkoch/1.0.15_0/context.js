...
init = () => {
	...
	st.textContent = '.audio_row:not(.audio_claimed):is(:hover,:has(.audio_row__actions)) .audio_row__info {margin-right: 29px} .audio_row:not(.audio_claimed):hover .audio_player__place {padding-right: 29px} .audio_row:has(.audioSize:hover) .audio_row__actions{position: fixed !important; left: 200vw !important}'
	...
	+'.audioSize {display: none; align-items: center; position: absolute; top: 0; right: 5px; width: 24px; height: 100%; &:hover{padding-left: 3px}}'
...
updateNode = e => {
	if(e.A || /claimed|radio/.test(e.className))return;
...
getUrl = (ids, h) => {
	get('/music', 'al=1&act=reload_audio'+(ids.split('_').length<4?'s&audio_':'&')+'ids='+ids, j => {
