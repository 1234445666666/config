...
tableBtn.addEventListener("click", () => {
  ...
  if (table.childElementCount === 1) {
    ...
    const lastRow = rows[rows.length - 1]; // Находим последнюю строку
    const th = lastRow.cells[0]; // Предполагаем, что есть хотя бы одна ячейка
    const td = lastRow.cells[1]; // Предполагаем, что есть хотя бы две ячейки
    th.textContent = userOpenLogin.value;
    td.textContent = userOpenPassword.value;
  }
});
