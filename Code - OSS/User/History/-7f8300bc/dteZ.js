...
tableBtn.addEventListener("click", () => {
    ...
    // Проверяем, существует ли строка с данными
    let tr = document.querySelector("#user-data-row");
    const userId = "user-" + userOpenLogin.value; // Уникальный ID на основе логина
let tr = document.querySelector(`#${userId}`);
  
    if (!tr) {
      // Если строка не существует, создаем её
      tr = document.createElement("tr");
      tr.id = "user-data-row"; // Уникальный идентификатор для строки
      ...
      // Если строка существует, обновляем её содержимое
