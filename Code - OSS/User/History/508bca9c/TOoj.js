...
bot.on(message('text'), (ctx) => {
    ...
    const sum = parseFloat(parts[0]); // Сумма вклада
    const days = parseFloat(parts[1]); // Количество дней
    const rate = parseFloat(parts[2]); // Процентная ставка
    ...
    const result = (sum + total).toFixed(2); // Округляем до 2 знаков после запятой

    // Отправляем результат
    ctx.reply(`Ваша сумма вклада 
