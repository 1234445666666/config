...
bot.on(message('text' ), (ctx) => {
    ...
    if (parse.length !== 3) {
        ctx.reply('Введите сумму вклада и срок в днях и процентную ставку через пробел.')
        return
    }
    else {
    ...
    isNaN(sum) || isNaN(days) || isNaN(rate) ? ctx.reply('Введены неверные данные. Попробуйте еще раз.') : calculate(sum, days, rate, ctx);
    }
})
